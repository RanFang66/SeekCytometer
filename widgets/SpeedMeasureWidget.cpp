...
void SpeedMeasureWidget::onCytometerSettingChanged(int settingId)
{
    comboPostCh->clear();
    ...
    comboPreCh->clear();
    for (const Detector &detector : DetectorsDAO().fetchDetectorsUnset(settingId)) {
...
void SpeedMeasureWidget::initWidget()
{
    ...
    layout->addWidget(new QLabel(tr("Two measurement point distance(μm): "), this), 3, 0);
    layout->addWidget(spinDist, 3, 1);
    layout->addWidget(btnConfirm, 4, 0, 1, 2);
    ...
    connect(btnConfirm, &QPushButton::clicked, this, [this](){
        ...
        emit speedMeasureSettingChangged(m_preChId, m_postChId, m_preChThresh, m_dist);
    });
}
